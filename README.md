...
Minimal starter README — project initialized.

## About

A Laravel-based subscription management service. Contains core models for plans, billing cycles, prices, subscriptions, and payments.
...
This README was reset to a minimal initial state per request.
# 📊 Subscription Lifecycle Engine
...
3. **plan_prices** - Multi-currency pricing (now the primary reference for subscription price and billing cycle)
4. **subscriptions** - Active subscriptions with state tracking (references `plan_price_id`)
5. **subscription_histories** - Audit trail of all state changes
6. **payments** - Unified payments ledger storing succeeded/failed/pending events
...
erDiagram
    ...
    PLANS ||--o{ PLAN_BILLING_CYCLES : has
    PLAN_BILLING_CYCLES ||--o{ PLAN_PRICES : has
    PLAN_PRICES ||--o{ SUBSCRIPTIONS : priced_by
