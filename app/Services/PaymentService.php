...
): Subscription
{
    ...
    public function handlePaymentFailure(
        ...
        return DB::transaction(function () use ($subscription, $errorReason, $errorCode, $errorMessage, $amount, $currency, $transactionId, $provider, $rawPayload) {

            $this->recordFailedPayment($subscription, $amount ?? $subscription->price->price, $currency ?? $subscription->price->currency, $errorReason, $errorCode, $errorMessage);

            $this->recordPayment($subscription, 'failed', 'charge', $amount, $currency, $transactionId, $provider, $rawPayload, $errorCode ? ['error_message' => $errorMessage, 'error_code' => $errorCode] : null);
    ...
    /**
     * Record a payment in the unified payments table
     */
    public function recordPayment(
        ...
        return Payment::create([
            ...
            'raw_payload' => $rawPayload ?? $metadata,
    ...
    public function processPendingRetries(): array
    {
        ...
        $retryCandidates = FailedPayment::where('recovered', false)
            ->where('failed_at', '>=', Carbon::now('UTC')->subDays(7)) 
